...
| `StrictNodeID` | `false` | Require the configured `NodeID` to be unique before startup. A conflict, unreachable seed, or seed without the check endpoint aborts startup instead of assigning a random replacement. |
...
Applications with operator-assigned node names should enable
`StrictNodeID`. A fresh node then verifies its configured name before a
full database copy, and an existing node verifies it again before opening
any listener or starting replication workers. Registered names cannot be
reused by fresh nodes; a restart can reclaim its persisted name only after
the previous process is no longer active. The seed must already support the
strict identity endpoint, otherwise startup fails with an upgrade message.
